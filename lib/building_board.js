...
function BuildingBoard(json) {
  ...
  if (json) {
    ...
    if (bb.max_points) {
      throw new Error("Max points is a protected attribute! Will not deserialize");
    }
    var pos, piece;
    ...
    for (var pos_key in bb.pieces) {
      if (bb.pieces.hasOwnProperty(pos_key)) {
        ...
        if (piece.colour !== first_colour) {
          throw new Error("BuildingBoards can only contain pieces of the same colour");
        }
...
BuildingBoard.prototype.removePiece = function(position) {
  ...
  if (!piece) {
    throw new Error("No piece at: " + position.asKey());
  }
