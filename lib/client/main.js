...
require([
  ...
  function updateIW() {
    ...
    $("#psyop_normal #value").text(pluralize(g_gameState.currentPsyOpAttackCost));
    $("#psyop_reinforced #value").text(pluralize(g_gameState.currentPsyOpDefendCost));
    $("#psyop_feint #value").text(pluralize(g_gameState.feintCost));
    $("#ew_normal #value").text(pluralize(g_gameState.currentEWAttackCost));
    $("#ew_reinforced #value").text(pluralize(g_gameState.currentEWDefendCost));
    $("#ew_feint #value").text(pluralize(g_gameState.feintCost));

    renderPointsRemaining($("#action_selector #points_indicator"), g_gameState.remainingIW);
  }
  ...
  function phaseHasChanged(old_phase, new_phase) {
    ...
    if (new_phase === phases.MOVE) {
      ...
      printMessage('server', "Choose one of psyop, electronic warfare, or end turn.");
  ...
  function updatePlayerTurnOverlay() {
    ...
    if (g_gameState.getWinner()) {
      ...
      setOverlayText($overlay, $flash, winner.name.toUpperCase() + " WINS");
      return;
    }
    if (isSpectator()) {
      setOverlayText($overlay, $flash, g_gameState.getCurrentPhase() + "'S TURN");
      return;
    }
    if (g_gameState.getCurrentRole() === g_role) {
      setOverlayText($overlay, $flash, yourTurn);
    } else {
      setOverlayText($overlay, $flash, opponentsTurn);
    }
  }
  ...
  socket.on('connect', function() {
    ...
    socket.on('update', function(updateResponse) {
      ...
      if (g_gameState.currentPhase === g_gameState.PHASES.PAWNUPGRADE &&
        ...
        g_gameState.currentRole == getPlayerColour()) {
        showIWDefenseDialog(g_gameState.current_iw_attack);
      }
      ...
      if (updateResponse.result.type === "capture" && updateResponse.result.attack === "psyop") {
