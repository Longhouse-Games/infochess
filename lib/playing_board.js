...
PlayingBoard.prototype.getRemainingIW = function(colour) {
  if (colour !== 'black' && colour !== 'white') {
    throw "Invalid value for colour";
  }
...
PlayingBoard.prototype.psyop_attack = function(attack) {
  if (!attack.attacker) {
    throw "Given IW attack has no attacker";
  }
  if (!attack.type || attack.type !== 'psyop') {
    throw "Invalid IW attack type: "+attack.type;
  }
  if (typeof attack.reinforced === 'undefined') {
    throw "Invalid IW parameters. Reinforced must be given";
  }
...
PlayingBoard.prototype.pawn_upgrade = function(colour, new_type) {
  ...
  this.pieces[pawn_pos.asKey()] = new Piece(new_type, colour);
  return {
    result: 'pawn_upgrade',
...
PlayingBoard.prototype.move = function(role, src, dest, state) {
  ...
  if (moving_piece.type === 'pawn' && !target && state === 'pawn-capture') {
    ...
    if (!this.isLastMoveEnPassant()) {
      throw "Invalid move for a pawn-capture. Not a valid target, and last move was not eligible for en passant.";
    }
    ...
    if (!target) {
      throw "Invalid move for en passant pawn capture. There's no pawn at that location";
    }
